...
const DOCUMENT_REQUIREMENTS = [
  { key: 'aadhaar_card', type: 'Aadhaar Card', label: 'Aadhaar Card', required: true, icon: FileText },
  { key: 'passport_photo', type: 'Passport-size Photograph', label: 'Passport-size Photograph', required: true, icon: ImageIcon },
  { key: 'education_certificate', type: 'Educational Certificate', label: 'Educational Certificates', required: true, icon: FileText },
  { key: 'bank_proof', type: 'Bank Passbook or Cancelled Cheque', label: 'Bank Passbook / Cancelled Cheque', required: true, icon: FileText },
  { key: 'pan_card', type: 'PAN Card', label: 'PAN Card', required: false, icon: FileText },
  { key: 'resume', type: 'Resume / CV', label: 'Resume / CV', required: false, icon: FileText },
  { key: 'category_certificate', type: 'Category Certificate', label: 'Category Certificate', required: false, icon: FileText },
];

export default function CandidateDocuments({ user, onUserUpdate, onSectionChange }) {
  ...
  const verifiedCount = DOCUMENT_REQUIREMENTS.filter(r => {
    return getDocumentStatus(r.type).toLowerCase() === 'verified';
  }).length;
  ...
  const getChecklistItem = (req) => {
    ...
    if (status === 'Verified') {
      return { symbol: '✓', color: 'text-emerald-500 font-bold', label: `${req.label} Verified` };
    }
  ...
  return (
    <div className="space-y-6 text-left relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Documents Center</h2>
          <p className="text-slate-500 font-semibold mt-1 text-xs">Upload required documents to complete your profile verification.</p>
        </div>
        <button
          ...
          {uploadedDocsCount === 0 ? (
            ...
            <div className="bg-white rounded-3xl border border-slate-200 py-16 flex flex-col items-center gap-3 shadow-xs">
              ...
              <div className="text-center">
                ...
                <p className="text-[11px] text-slate-400 font-semibold mt-0.5">Upload your required documents to begin verification.</p>
              </div>
              <button
                onClick={() => setIsUploadModalOpen(true)}
                        ...
                        {existing ? (
                          <>
                            <button
                              ...
                              className="h-8 px-3 border border-slate-200 hover:border-violet-200 bg-white text-slate-600 hover:text-[#6D3BFF] rounded-xl text-[10px] font-black transition flex items-center justify-center gap-1 cursor-pointer"
                            >
                            ...
                            <label className="relative">
                              ...
                              />
                              <div className="h-8 px-3 border border-violet-200 bg-violet-50 text-violet-700 hover:bg-violet-100 rounded-xl text-[10px] font-black transition flex items-center justify-center gap-1 cursor-pointer">
                      ...
                      <div className="mt-3 p-3 bg-rose-50 border border-rose-100 rounded-xl flex items-start gap-2 text-left">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[9px] font-semibold text-rose-600 mt-0.5">
                            Reason: {existing.comments || existing.rejection_reason || 'Image is unclear. Please upload a higher-quality document.'}
                          </p>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-5 shadow-xs text-left">
            ...
            <div className="flex items-center gap-4 mb-4 pt-1">
              ...
              <div>
                ...
                <p className="text-[9px] font-semibold text-slate-400 mt-0.5 leading-snug">Complete all required documents to proceed with verification.</p>
              </div>
            </div>
            ...
            <div className="space-y-2 border-t border-slate-100 pt-4">
              {DOCUMENT_REQUIREMENTS.map((item) => {
                ...
                return (
                  <div key={item.key} className="flex items-start gap-2 text-[10px] font-semibold text-slate-600">
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex gap-3">
              {previewUrl && (
                <a
                  ...
                  className="flex-1 h-10 border border-slate-200 hover:border-violet-200 text-slate-600 hover:text-[#6D3BFF] text-xs font-black rounded-xl bg-white transition flex items-center justify-center gap-1"
                >
