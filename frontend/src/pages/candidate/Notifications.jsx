...
const getIconInfo = (type) => {
  switch (type) {
    case 'application': case 'applications':
      return { icon: Briefcase, bg: 'bg-violet-50 text-[#6D3BFF] border-violet-100' };
    case 'interview':
    ...
    case 'profile_update': case 'registration':
      return { icon: User, bg: 'bg-blue-50 text-blue-600 border-blue-100' };
    case 'hired': case 'status_change':
    ...
    case 'active_apprentice':
      return { icon: GraduationCap, bg: 'bg-green-50 text-green-600 border-green-100' };
    case 'stipend':
      return { icon: CheckCircle2, bg: 'bg-teal-50 text-teal-600 border-teal-100' };
    default:
...
const TYPE_TAB_MAP = {
  ...
  'Documents': ['document'],
  'Contracts': ['contract', 'active_apprentice', 'hired'],
  'System': ['system', 'registration', 'profile_update', 'stipend']
};
...
export default function CandidateNotifications({ onSectionChange }) {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/candidate/notifications?limit=100`, {
  ...
  const markOneRead = async (id) => {
    try {
      await fetch(`${API_BASE}/candidate/notifications/${id}/read`, {
        method: 'PATCH',
        headers: getAuthHeaders()
      });
  ...
  const markAllAsRead = async () => {
    try {
      await fetch(`${API_BASE}/candidate/notifications/read-all`, {
        method: 'PATCH',
        headers: getAuthHeaders()
      });
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xs p-5 text-left">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2 border-b border-slate-100">
                ...
                <span className="text-xs font-black text-slate-800">{notifications.filter(n => ['application','applications'].includes(n.notification_type || n.type)).length}</span>
              </div>
              <div className="flex items-center justify-between py-2">
                <span className="text-[11px] font-bold text-slate-500">Contracts</span>
                <span className="text-xs font-black text-slate-800">{notifications.filter(n => ['contract','active_apprentice','hired'].includes(n.notification_type || n.type)).length}</span>
              </div>
            </div>
            {onSectionChange && (
              <button
              ...
              >
                My Applications <ArrowRight size={12} />
              </button>
            )}
          </div>
        </div>
