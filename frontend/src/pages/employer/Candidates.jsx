  ...
  return (
    <div className="space-y-6 text-left selection:bg-violet-100 selection:text-violet-950 pb-12 w-full max-w-full overflow-x-hidden">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xs overflow-hidden w-full relative">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/70 border-b border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  <th className="py-4 px-5 w-[32%]">Candidate Info</th>
                  <th className="py-4 px-4 w-[20%]">Applied For</th>
                  <th className="py-4 px-4 w-[16%]">Qualification</th>
                  <th className="py-4 px-4 w-[10%]">Experience</th>
                  <th className="py-4 px-4 w-[14%]">Application Date</th>
                  <th className="py-4 px-4 w-[12%]">Status</th>
                  <th className="py-4 px-4 w-[14%]">Current Stage</th>
                  <th className="py-4 px-5 text-right w-[60px]">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-150">
                {currentRows.map((cand) => {
                  ...
                  const fmtTime = dateObj.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });

                  const initials = cand.name.trim().split(/\s+/).slice(0, 2).map(n => n[0]).join('').toUpperCase();

                  return (
                    <tr key={cand.id} className={`hover:bg-slate-50/40 transition-colors cursor-pointer ${isSelected ? 'bg-violet-50/50' : ''}`}
                      onClick={() => setSelectedCandidate(cand)}
                    >
                      {/* Candidate Name & Contact Details combined */}
                      <td className="py-4.5 px-5">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full font-black text-[10px] flex items-center justify-center shrink-0 border ${isSelected ? 'bg-[#6D3BFF] text-white border-[#6D3BFF]' : 'bg-violet-100 text-[#6D3BFF] border-violet-200'}`}>
                            {initials}
                          </div>
                          <div className="min-w-0">
                            <p className="text-xs font-black text-slate-800 leading-none truncate">{cand.name}</p>
                            <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 mt-1.5 text-[9px] text-slate-400 font-bold select-all leading-none">
                              <span className="flex items-center gap-0.5 truncate max-w-[140px] md:max-w-none">
                                <Mail size={9} className="text-slate-350 shrink-0" /> {cand.email}
                              </span>
                              <span className="hidden sm:inline text-slate-300">•</span>
                              <span className="flex items-center gap-0.5 whitespace-nowrap">
                                <Phone size={9} className="text-slate-355 shrink-0" /> {cand.phone}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col min-w-0">
                          <span className="text-xs font-bold text-slate-750 leading-tight truncate">{cand.appliedFor}</span>
                          {cand.jobCode && (
                            <span className="text-[9px] font-mono font-bold text-slate-400 mt-0.5">{cand.jobCode}</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex flex-col min-w-0">
                          <span className="text-xs font-bold text-slate-705 leading-tight truncate">{cand.qualification}</span>
                          {cand.passingYear && (
                            <span className="text-[9px] text-slate-400 font-semibold mt-0.5">Class of {cand.passingYear}</span>
                          )}
                        </div>
                      </td>
                      ...
                      {/* Experience */}
                      <td className="py-4 px-4">
                        <span className="text-xs font-semibold text-slate-600 whitespace-nowrap">{cand.experience}</span>
                      </td>

                      {/* Application Date */}
                      <td className="py-4 px-4">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-extrabold text-slate-705 flex items-center gap-1 whitespace-nowrap">
                            <Calendar size={11} className="text-slate-400 shrink-0" /> {fmtDate}
                          </span>
                          <span className="text-[9px] text-slate-400 font-semibold mt-0.5 pl-4 whitespace-nowrap">{fmtTime}</span>
                        </div>
                      </td>
                      ...
                      {/* Current Stage */}
                      <td className="py-4 px-4">
                        <span className="text-xs font-black text-slate-805 truncate block max-w-[120px]">{cand.currentStage}</span>
                      </td>
                      ...
                      <td className="py-4 px-5 text-right relative" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => setSelectedCandidate(cand)}
                            className={`w-7 h-7 rounded-lg flex items-center justify-center transition cursor-pointer ${isSelected ? 'bg-[#6D3BFF] text-white' : 'hover:bg-violet-50 text-slate-400 hover:text-[#6D3BFF]'}`}
                          ...
                          <div className="relative">
                            <button
                              onClick={() => setActiveDropdown(isDropdownActive ? null : cand.id)}
                            ...
                            {isDropdownActive && (
                              <>
                                ...
                                <div className="absolute right-0 mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-40 text-left animate-fade-in">
                                  <button
                                    onClick={() => {
                                      showToast?.(`Downloading resume for ${cand.name}...`, 'info');
                                      setActiveDropdown(null);
                                    }}
                                  ...
                                  <button
                                    onClick={() => handleStatusUpdate(cand.id, 'Shortlisted', 'Shortlisted')}
                                    className="w-full px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50 hover:text-[#6D3BFF] flex items-center gap-2 cursor-pointer"
                                  >
                                    <CheckCircle2 size={12} className="text-emerald-500" /> Shortlist Candidate
                                  </button>
                                  <button
                                    onClick={() => handleStatusUpdate(cand.id, 'Interview Scheduled', 'Interview')}
                                    className="w-full px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50 hover:text-[#6D3BFF] flex items-center gap-2 cursor-pointer"
                                  >
                                    <Calendar size={12} className="text-blue-500" /> Schedule Interview
                                  </button>
                                  <button
                                    onClick={() => handleStatusUpdate(cand.id, 'Selected', 'Offer Stage')}
                                    className="w-full px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50 hover:text-[#6D3BFF] flex items-center gap-2 cursor-pointer"
                                  >
                                    <BadgeCheck size={12} className="text-teal-500" /> Move to Offer
                                  </button>
                                  <button
                                    onClick={() => handleStatusUpdate(cand.id, 'Joined', 'Joined')}
                                    className="w-full px-3 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50 hover:text-[#6D3BFF] flex items-center gap-2 cursor-pointer"
                                  >
                                  ...
                                  <div className="h-px bg-slate-100 my-1" />
                                  <button
                                    onClick={() => handleStatusUpdate(cand.id, 'Rejected', 'Rejected')}
          ...
          {/* Bottom Pagination */}
          <div className="bg-slate-50/50 border-t border-slate-200 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-bold text-slate-500 w-full">
            <span>
            ...
            <div className="flex items-center gap-4 flex-wrap justify-end">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="w-8 h-8 rounded-lg border border-slate-250 flex items-center justify-center bg-white text-slate-500 hover:bg-slate-50 disabled:opacity-50 transition cursor-pointer"
                >
                  <ChevronLeft size={14} />
                </button>
                  ...
                  return (
                    <button
                      ...
                      className={`w-8 h-8 rounded-lg border text-[10px] font-black flex items-center justify-center transition cursor-pointer ${
                        isActive
                          ? 'bg-[#6D3BFF] border-[#6D3BFF] text-white shadow-sm shadow-violet-100'
    ...
    <div className="fixed inset-0 z-[120] bg-slate-900/40 backdrop-blur-sm flex justify-end">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-4 border-b border-slate-100">
            ...
            <div className="space-y-2.5">
              ...
              {cand.passingYear && <InfoRow label="Passing Year" value={`Class of ${cand.passingYear}`} />}
              {cand.institute && <InfoRow label="Institute" value={cand.institute} />}
              {cand.percentage && <InfoRow label="Percentage / Grade" value={`${cand.percentage}%`} />}
            </div>
          </div>
        ...
        <div className="shrink-0 border-t border-slate-200 bg-slate-50/50 p-4 flex items-center gap-2">
          <button
            onClick={() => showToast?.(`Downloading resume for ${cand.name}...`, 'info')}
