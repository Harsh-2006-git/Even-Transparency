...
export default function EmployerSettings({ user, onSectionChange, onUserUpdate }) {
  ...
  const handleDeleteAccount = () => {
    ...
    setModalType(null);
    setConfirmInput('');
    triggerToast('Your delete request has been filed. All company data will be removed within 24 hours.');
  };
  ...
  return (
    <div className={`space-y-6 ${accessibility.largerText ? 'text-sm' : 'text-xs'} ${accessibility.highContrastMode ? 'contrast-125 saturate-150' : ''}`}>
      ...
      <div className="flex overflow-x-auto gap-2 no-scrollbar pb-1.5 md:pb-0">
        {['Account', 'Security', 'Notifications', 'Privacy', 'Hiring', 'Support', 'Danger Zone'].map((tab) => {
      ...
      <div ref={dangerRef} className="rounded-3xl border border-rose-200/80 bg-rose-50/40 p-6 shadow-xs text-left transition hover:shadow-sm">
        <div className="flex items-center gap-3 border-b border-rose-100 pb-4 mb-5">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-rose-600/80 mt-0.5">Actions that affect company account status.</p>
          </div>
        </div>
