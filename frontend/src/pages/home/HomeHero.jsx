...
export default function HomeHero({ onOpenDemoModal }) {
  // 7 lifecycle stages: Exact original coordinates and alignment
  const stages = [
    {
      id: 1,
      title: "Mobilization",
  ...
  return (
    ...
    >
      ...
      <div className="flex-none pt-3 sm:pt-4 md:pt-5 px-4 flex flex-col items-center text-center z-20 max-w-4xl mx-auto">
        <h1 className="text-black font-kaiseiTokumin text-[24px] min-[380px]:text-[27px] sm:text-[36px] lg:text-[44px] xl:text-[48px] font-extrabold leading-[1.16] tracking-tight max-w-[820px] mb-2 sm:mb-2.5">
          <span className="block whitespace-nowrap">One Platform for the Entire</span>
          <span className="block whitespace-nowrap text-[#FF408A]">Candidate Journey</span>
        </h1>
        <p className="text-[#787878] font-inter text-[11px] min-[380px]:text-[12px] sm:text-sm md:text-[14px] leading-relaxed max-w-[700px] px-2">
        ...
        <div className="mt-2.5 sm:mt-3 flex items-center justify-center">
          <button
            ...
            className="cursor-pointer px-6 sm:px-7 py-2 sm:py-2.5 rounded-full bg-black hover:bg-[#1a1a1a] text-white font-inter text-sm sm:text-base font-bold shadow-md hover:shadow-lg transition-all duration-200"
          >
            Get Started
          </button>
        </div>
      ...
      {/* Desktop S-Curve View (Preserved in exact original high elevation & curve geometry) */}
      ...
      <div className="lg:hidden w-full my-6 sm:my-8 flex flex-col items-center z-10 overflow-hidden relative">
        ...
        <div className="w-full relative py-8">
          ...
          <div 
            className="absolute top-[48px] sm:top-[52px] left-0 right-0 h-[8px] sm:h-[10px] w-full z-0 pointer-events-none shadow-sm rounded-full"
            style={{
              background: 'linear-gradient(90deg, #250914 0%, #7B1D45 25%, #8E1C4E 50%, #DB2F74 75%, #FF408A 100%)'
            }}
          />

          {/* Infinite Marquee Track with enlarged connected nodes */}
          <div className="animate-marquee flex items-start gap-7 sm:gap-9">
            
            {/* Set 1 */}
            {stages.map((stage) => (
              <div
                key={`m1-${stage.id}`}
                className="shrink-0 w-[210px] sm:w-[230px] flex flex-col items-center text-center relative z-10 group"
              >
                {/* Enlarged Circular Node centered on the main line */}
                <div className="w-[72px] h-[72px] sm:w-[80px] sm:h-[80px] rounded-full border-[2.5px] border-dashed border-[#FF408A] bg-white shadow-lg flex items-center justify-center p-2.5 transition-transform group-hover:scale-105">
                  <img
                    src={stage.img}
                    alt={stage.title}
                ...
                {/* Step badge */}
                <span className="mt-2 px-3 py-0.5 rounded-full bg-[#FFF8FA] text-[10.5px] font-extrabold text-[#FF408A] border border-[#FF408A]/40 shadow-xs">
                  Stage {stage.id}
                </span>

                {/* Description Card directly below */}
                <div className="w-full mt-2.5 bg-white/95 backdrop-blur-xs p-3.5 rounded-2xl border border-slate-200/90 shadow-sm">
                  <h3 className="text-slate-900 font-kaiseiTokumin text-sm sm:text-base font-extrabold leading-tight group-hover:text-[#FF408A] transition-colors">
                    {stage.title}
                  </h3>
                  <p className="text-slate-600 font-inter text-xs sm:text-[12.5px] leading-snug mt-1 line-clamp-2">
                    {stage.subtitle}
                  </p>
                </div>
              </div>
      ...
      <div
        className="flex-none w-full py-2.5 sm:py-3.5 px-0 sm:px-8 md:px-16 flex items-center justify-center z-20 shadow-lg overflow-hidden relative"
      ...
      >
          ...
          {stripFeatures.map((item, idx) => (
            <React.Fragment key={`desk-feat-${item.id}`}>
              <div className="flex items-center justify-center gap-3 flex-1 px-4">
                <div className="shrink-0 w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#FF408A] shadow-xs">
                  {item.icon}
                </div>
                <span className="text-white font-inter text-sm font-bold whitespace-nowrap">
                  {item.title}
                </span>
              </div>
        ...
        <div className="lg:hidden w-full overflow-hidden relative py-0.5">
          <div className="animate-marquee flex items-center gap-6 sm:gap-8">
            
            {/* Set 1 */}
            {stripFeatures.map((item) => (
              <div key={`m-feat-1-${item.id}`} className="flex items-center gap-2 shrink-0">
                <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-[#FF408A] shadow-xs shrink-0">
                  {item.icon}
                </div>
                <span className="text-white font-inter text-xs font-bold whitespace-nowrap">
                  {item.title}
                </span>
              </div>
            ))}
            ...
            {/* Set 2 */}
            {stripFeatures.map((item) => (
              <div key={`m-feat-2-${item.id}`} className="flex items-center gap-2 shrink-0">
                <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-[#FF408A] shadow-xs shrink-0">
                  {item.icon}
                </div>
                <span className="text-white font-inter text-xs font-bold whitespace-nowrap">
            ...
            {/* Set 3 */}
            {stripFeatures.map((item) => (
              <div key={`m-feat-3-${item.id}`} className="flex items-center gap-2 shrink-0">
                <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-[#FF408A] shadow-xs shrink-0">
                  {item.icon}
                </div>
                <span className="text-white font-inter text-xs font-bold whitespace-nowrap">
